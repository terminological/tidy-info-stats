...
#debug(calculateEntropy_Grassberger)
...
s %>% calculateEntropy_Histogram(groupVars=vars(x_discrete), mm=TRUE)
s %>% calculateEntropy_InfoTheo(groupVars=vars(x_discrete), infoTheoMethod="mm")
s %>% calculateEntropy_Grassberger(groupVars=vars(x_discrete))
...
devtools::load_all("..")
# debug(calculateEntropy_MontgomerySmith)
...
for (dist in dists) {
  ...
  for (i in c(1:10)) { #0)) {
    for (j in c(1:10,20,40,60,80,100)) { #,15,20,25,30,35,40,45,50,60,70,80,90,100)) {
      ...
      for (m in method) {
        estimate = calculateEntropy(s, groupVars=vars(x_discrete), method = m, infoTheoMethod = "emp", mm=FALSE) %>% pull(I)
...
results = results %>% rbind(testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousMI(vars(outcome), value, method = "SGolay"))
...
results = results %>% rbind(testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousMI(vars(outcome), value, method = "DiscretiseByRank"))
results = results %>% rbind(testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousMI(vars(outcome), value, method = "DiscretiseByValue", discreteMethod="Entropy", entropyMethod="InfoTheo"))
results = results %>% rbind(testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousMI(vars(outcome), value, method = "DiscretiseByValue", discreteMethod="Histogram"))
results = results %>% rbind(testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousMI(vars(outcome), value, method = "DiscretiseByValue", discreteMethod="MontgomerySmith"))
results = results %>% rbind(testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousMI(vars(outcome), value, method = "DiscretiseByValue", discreteMethod="Entropy", entropyMethod="Grassberger"))
...
```{r}
devtools::load_all("..")
testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousConditionalMI(vars(outcome), value, method = "KWindow")
testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousConditionalMI(vars(outcome), value, method = "Kernel")
#debug(calculateDiscreteContinuousConditionalMI_Entropy)
testData %>% group_by(test) %>% tidyinfostats::calculateDiscreteContinuousConditionalMI(vars(outcome), value, method = "Entropy", entropyMethod="Quantile")
```